...
__device__ double calLocalScore_kernel(int * dev_valuesRange, int *dev_samplesValues, int samplesNum, int* parentSet, int size, int curNode, int nodesNum){
	...
	for (i = 0; i < samplesNum; i++){
		pvalue_base = 0;
		for (j = 0; j < size - 1; j++){
			pvalue_base = pvalue_base + 1;
			pvalue_base = pvalue_base * dev_valuesRange[parentSet[j + 1] - 1];
		}
		pvalue_base = pvalue_base + 1;
		...
		if (size > 0){
			for (j = 0; j < size - 1; j++){
				pvalue_index = pvalue_index + dev_samplesValues[i * nodesNum + parentSet[j] - 1];
				pvalue_index = pvalue_index * dev_valuesRange[parentSet[j + 1] - 1];
			}
			pvalue_index = pvalue_index + dev_samplesValues[i * nodesNum + parentSet[j] - 1];
			pvalue_index = pvalue_index - pvalue_base;
		}

		N[pvalue_index * dev_valuesRange[curNode] + dev_samplesValues[i * nodesNum + curNode] - 1]++;
	...
	for (i = 0; i < valuesNum; i++){
		sum = 0;
		for (j = 0; j < dev_valuesRange[curNode]; j++){
			cur = i * dev_valuesRange[curNode] + j;
			if (N[cur] != 0)
			{
				localScore = localScore + lgamma(N[cur] + alpha) - lgamma(alpha);
				sum = sum + N[cur];
			}
		}
		localScore = localScore + lgamma(ALPHA / valuesNum) - lgamma(ALPHA / valuesNum + sum);
	}
...
__global__ void calcAllPossibleLocalScore_kernel(int *dev_valuesRange, int *dev_samplesValues, double *dev_lsTable, int samplesNum, int nodesNum, int parentSetNum) {
	...
	int limit = nodesNum * parentSetNum;
	if (id < limit){
		int nodeId = id / parentSetNum;
		int parentSetId = id % parentSetNum;
		...
		dev_lsTable[id] = calLocalScore_kernel(dev_valuesRange, dev_samplesValues, samplesNum, parentSet, size, nodeId, nodesNum);
	}
}
...
__global__ void generateOrders_kernel(int *dev_newOrders, curandState *dev_curandState, int nodesNum){
	...
	if (threadIdx.x != 0){
		...
		dev_newOrders[threadIdx.x * nodesNum + i] = dev_newOrders[threadIdx.x * nodesNum + j];
...
__global__ void calcOnePairPerThread_kernel(double * dev_lsTable, int * dev_newOrders, double * dev_parentSetScore, int nodesNum, int parentSetNum, int parentSetNumInOrder){
	...
	for (nodePos = 0; nodePos < nodesNum; nodePos++){
		for (parentSetSize = 0; parentSetSize <= CONSTRAINTS && parentSetSize < nodePos + 1; parentSetSize++){
	...
	// find combination
	findCombWithSize_kernel(nodePos + 1, parentSetIndex, parentSetSize, parentSet);
	...
	dev_parentSetScore[id] = dev_lsTable[(curNode - 1)*parentSetNum + i];
}
...
__global__ void calcMaxParentSetScoreForEachNode_kernel(double *dev_parentSetScore, double *dev_maxLocalScore, int parentSetNumInOrder, int nodesNum){
	...
	double max = dev_parentSetScore[threadIdx.x * parentSetNumInOrder + parentSetStartPos];
